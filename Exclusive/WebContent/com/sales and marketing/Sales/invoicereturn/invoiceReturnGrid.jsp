          ...
          applyFilter(dataField,$(this).val());  
            ...
            var source =
            {
                ...
                pager: function (pagenum, pagesize, oldpagenum) {
                    ...
                    $("#jqxInvoiceReturn").on("bindingcomplete", function (event) { 
                        ...
                        if($('#mode').val()=="A"){
                     	 ...
                     	 var rows = $("#jqxInvoiceReturn").jqxGrid('getrows');   
        	             	  ...
        	             	  for(var i=0;i<rows.length;i++){
        	             	   		var netotal=$('#jqxInvoiceReturn').jqxGrid('getcellvalue', i, "netotal"); 
        	                		
        	                		  var taxper= $('#jqxInvoiceReturn').jqxGrid('getcellvalue', i, "taxper"); 
        	                		  if(parseFloat(taxper)>0)
        	                			  {
        	                		  var taxempamount=parseFloat(netotal)*(parseFloat(taxper)/100);
        	                		  
        	                		  
        	                		  $('#jqxInvoiceReturn').jqxGrid('setcellvalue', i, "taxperamt",taxempamount);
        	                		  
        	                		  var taxtotalamount=parseFloat(netotal)+parseFloat(taxempamount);
        	                		  
        	                		  $('#jqxInvoiceReturn').jqxGrid('setcellvalue', i, "taxamount",taxtotalamount);
        	                			  }
        	                		  else
        	                			  {
        	                			  $('#jqxInvoiceReturn').jqxGrid('setcellvalue', i, "taxamount",netotal);
        	                			  }
            ...
            $("#jqxInvoiceReturn").jqxGrid(
            {
  	         ...
  	         $('#jqxInvoiceReturn').jqxGrid('setcellvalue', rowindex1, "brandname" ,rows[0].brandname);
  	              ...
  	              $('#jqxInvoiceReturn').jqxGrid('setcellvalue', rowindex1, "foc" ,rows[0].foc );
  	            $('#jqxInvoiceReturn').jqxGrid('setcellvalue', rowindex1, "oldfoc" ,rows[0].oldfoc );
        	  ...
        	  $('#jqxInvoiceReturn').jqxGrid('setcellvalue', rowindex1, "taxper" ,rows[0].taxper );
        	  $('#jqxInvoiceReturn').jqxGrid('setcellvalue', rowindex1, "netotal" ,rows[0].netotal);
            	...
            	$('#jqxInvoiceReturn').jqxGrid('setcellvalue', rowindex1, "detdocno" ,rows[0].detdocno );
            	 ...
            	 $('#jqxInvoiceReturn').jqxGrid('setcellvalue', rowindex1, "balfoc" ,rows[0].balfoc);
            ...
            if (cell4 != undefined && cell4.datafield == 'productname') {
                ...
                columns: [
							...
							{ text: 'Quantity', datafield: 'qty', width: '5%',cellclassname: cellclassname },
							...
							{ text: 'Discount', datafield: 'dis', width: '8%', cellsformat: 'd2', cellsalign: 'right', align: 'right',aggregates: ['sum1'],aggregatesrenderer:rendererstring1,cellclassname: cellclassname,editable:false},
            ...
            $("#jqxInvoiceReturn").on('cellvaluechanged', function (event) 
                ...
                $('#sidesearchwndow').jqxWindow('close');
            	   ...
            	   if(datafield=="productname")
            	  	 ...
            	  	 if(datafield=='qty' || datafield=='foc'){
           	 	...
           	 	if(datafield=="netotal")
           		    {
            	 	   ...
            	 	   document.getElementById("orderValue").value=summaryData.sum.replace(/,/g,''); 
            	     		...
            	     		var netotal=$('#jqxInvoiceReturn').jqxGrid('getcellvalue', rowBoundIndex, "netotal"); 
            	     		
            	     		
            	     	 
                  		  var taxper= $('#jqxInvoiceReturn').jqxGrid('getcellvalue', rowBoundIndex, "taxper"); 
                  		 if(parseFloat(taxper)>0)
                  			 {
                  		  var taxempamount=parseFloat(netotal)*(parseFloat(taxper)/100);
                  		  $('#jqxInvoiceReturn').jqxGrid('setcellvalue', rowBoundIndex, "taxperamt",taxempamount);
                  		  var taxtotalamount=parseFloat(netotal)+parseFloat(taxempamount);
                  		  
                  		  $('#jqxInvoiceReturn').jqxGrid('setcellvalue', rowBoundIndex, "taxamount",taxtotalamount);
                  			 }
                  		 else
                  			 {
                  			  $('#jqxInvoiceReturn').jqxGrid('setcellvalue', rowBoundIndex, "taxamount",netotal);
                  			 }
           ...
           if (x.readyState==4 && x.status==200)
            {
              ...
              if(parseInt(items)>0)
               {
            	  ...
            	  $('#jqxInvoiceReturn').jqxGrid('showcolumn', 'taxamount');
                  ...
                  else
              {
                	  ...
                	  $('#jqxInvoiceReturn').jqxGrid('hidecolumn', 'taxamount');

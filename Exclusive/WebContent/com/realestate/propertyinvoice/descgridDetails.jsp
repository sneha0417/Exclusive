...
var list =['GL','HR'];
if(id>0){
	invoicedata='<%=viewDAO.reloadPropertyInvoiceGrid(id, docno)%>';  
}
    ...
    var source =
    {
        ...
        deleterow: function (rowid, commit) {
	        ...
	        for(var i=0;i<detailrows.length;i++){
	        	if($.isNumeric(detailrows[i].nettotal) && parseInt(detailrows[i].account)==17903){
				...
				cellbeginedit: function (row) {
		            if (document.getElementById("termsval").value=="1")
		            {
		            	return false;
							...
							{ text: 'Grand Total', datafield: 'taxamount', width: '6%', cellsformat: 'd2', cellsalign: 'right', align: 'right'  ,aggregates: ['sum'],aggregatesrenderer:rendererstring ,editable:false },
							...
							{ text: 'Qutval', datafield: 'qutval', width: '0%',editable: true,hidden:true},
							{ text: 'nuprice', datafield: 'nuprice', width: '0%',cellsformat:'d2',hidden:true,editable: true},
							{ text: 'Head doc', datafield: 'headdoc', width: '0%' ,editable: true,hidden:true},     /* for account name, acccount from my head then save this number in table */
							{ text: 'Cost id', datafield: 'costtype', width: '0%',hidden:true ,editable: true},
							{ text: 'grtype', datafield: 'grtype', width: '0%',editable: true,hidden:true },  /* this for grrtype=4 and 5 then take cost code and costtype  */
							{ text: 'rowno', datafield: 'rowno', width: '0%',editable: true,hidden:true },  
          ...
          if(termsval>0)
    		    ...
    		    if(datafield=="costtype")
    		    {
		    		...
		    		if(datafield=="taxper")
		    		{
		           		...
		           		if(parseFloat(tper)>0){
		            		  ...
		            		  taxempamount=taxempamount.toFixed(2);
					 ...
					 if(datafield=="nettotal")
            		 {
            		  ...
            		  if(datafield=="taxamount" || datafield=="account"){
					        var totalamount=0.0;
					        var detailrows=$('#nidescdetailsGrid').jqxGrid('getrows');
					        for(var i=0;i<detailrows.length;i++){
								// && parseInt(detailrows[i].account)==17903
					        	if($.isNumeric(detailrows[i].nettotal) ){
					        		totalamount+=parseFloat(detailrows[i].nettotal);
					            } 
					        }
					        var agentrows=$('#agentGridId').jqxGrid('getrows');
            ...
            $("#nidescdetailsGrid").on('cellclick', function (event) 
            {
          	...
          	$("#nidescdetailsGrid").on('cellvaluechanged', function (event) {
                ...
                var taxpercent = $('#nidescdetailsGrid').jqxGrid('getcellvalue', rowIndex, "taxper");
          		if(dataField=="taxper"){    
          			if(taxpercent != 5 && taxpercent != 0){ 
          		        $("#nidescdetailsGrid").jqxGrid('showvalidationpopup', rowIndex, "taxper", "Tax Percentage should be either 0 or 5.");
          		         return true;  
          		        } else{  
              		        $("#nidescdetailsGrid").jqxGrid('hidevalidationpopups');
             		        return false;  
             		        }
          		}        	
          	});

...
<script type="text/javascript">
        ...
        $(document).ready(function () { 
           ...
           // create context menu
              ...
              $("#jqxBankPayment").on('contextmenu', function () {
                  return false;
              });
              
              $("#Menu").on('itemclick', function (event) {
           	   var args = event.args;
                  var rowindex = $("#jqxBankPayment").jqxGrid('getselectedrowindex');
                  if ($.trim($(args).text()) == "Edit Selected Row") {
                      
                  }
                  else {
                      var rowid = $("#jqxBankPayment").jqxGrid('getrowid', rowindex);
                      $('#jqxBankPayment').jqxGrid('setcellvalue', rowid , "amount1" ,"0.00");
               	   	  $('#jqxBankPayment').jqxGrid('setcellvalue', rowid , "baseamount1" ,"0.00");
                      $("#jqxBankPayment").jqxGrid('deleterow', rowid);
                  }
              });
              $("#jqxBankPayment").on('rowclick', function (event) {
                  if (event.args.rightclick) {
       		   if(document.getElementById("mode").value=="A" || document.getElementById("mode").value=="E" || document.getElementById("mode").value=="EDIT"){
                      $("#jqxBankPayment").jqxGrid('selectrow', event.args.rowindex);

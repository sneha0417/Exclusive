...
$(document).ready(function () {  
	var source =
    {
        ...
        datafields: [
			...
			{name : 'paymenttrno',type:'number'},
			{name : 'tenantacno',type:'number'},
			{name : 'brvdocno',type:'number'}
		],
    ...
    $("#contractGrid").jqxGrid(
    {
		...
		columns: [ 
			...
			{ text: 'Payment TRNO', datafield: 'paymenttrno', width: '6%',hidden:true },
			{ text: 'BRV Doc No',datafield: 'brvdocno',width:'6%',hidden:true},
			{ text: 'Tenant Acno', datafield: 'tenantacno', width: '6%',hidden:true },
    ...
    $('#contractGrid').on('rowdoubleclick', function (event) {    
    	...
    	$('#docno').val(docno);
    	$('#paymenttrno').val($('#contractGrid').jqxGrid('getcellvalue',rowindex,'paymenttrno'));
    	$('#hidexistingbrv').val($('#contractGrid').jqxGrid('getcellvalue',rowindex,'paymenttrno'));
    	$('#existingbrv').val($('#contractGrid').jqxGrid('getcellvalue',rowindex,'brvdocno'));
    	$('#tenantacno').val($('#contractGrid').jqxGrid('getcellvalue',rowindex,'tenantacno'));
    });

...
<title>Gateway ERP(Integrated) Copyright &#169; 2017 GW
	INNOVATIONS PVT. LTD.</title>
...
<script type="text/javascript">
		...
		$(document).ready(function () {			
			var arr=[ "Ford", "BMW", "Fiat" ];
			 /* $(function() { */
		          
		            	
		             /*  function(request, response) {
		             $.ajax({
		                    url : "getMenuForSearch.jsp",
		                    type : "GET",
		                    async : false,
		                    data : {
...
@font-face {
	...
	src: local('Myriad Pro'),
		url('../fonts/Myriad-Pro-Black-Italic_31618.ttf)') format('truetype');
}
...
.company {
   ...
   width:auto;*/
	...
	-webkit-transform: rotateX(1.7188733853924696deg)
		rotateY(1.7188733853924696deg);
...
button.bicon:disabled {
	opacity: 0.5;
}

;
button.iconym:disabled {
	opacity: 0.5;
}

;
button.iconnm:disabled {
	opacity: 0.5;
}
...
label.licenceExpiry {
	...
	background: -webkit-linear-gradient(left, rgba(255, 0, 0, 1) 0%,
		rgba(255, 0, 0, 1) 99%);
...
label.licenceExpiry:hover {
	...
	background: -webkit-linear-gradient(left, rgba(255, 0, 0, 1) 0%,
		rgba(255, 255, 0, 1) 19%, rgba(0, 255, 0, 1) 38%, rgba(0, 255, 255, 1)
		51%, rgba(0, 0, 255, 1) 67%, rgba(255, 0, 255, 1) 83%,
		rgba(255, 0, 0, 1) 99%); /* Chrome10+,Safari5.1+ */
	-webkit-background-clip: text;
	color: transparent;
	...
	<div id="mainBG" class="homeContent" data-type="background"
		...
		<div>
				...
				<ul>
					<s:iterator var="first" status="status" value="%{#request.MenuMap}">
						<li><s:property value="key"></s:property>
							<ul>
								<s:iterator var="second" status="status1"
									value="%{#first.getValue()}">
									<a href="#" onClick="geturl('<s:property value='key'/>')">
										<li><s:property value='key'></s:property>
											<ul>
												<s:iterator var="al" status="status1"
													value="%{#second.getValue()}">
													<a href="#" onClick="geturl('<s:property value='key'/>')">
														<li><label> <s:property value='key'></s:property>
														</label>
															<ul>
																<s:iterator var="al1" status="status1"
																	value="%{#al.getValue()}">
																	<a href="#"
																		onClick="geturl('<s:property value='al1'/>')"><label><li>
																				<s:property value='al1' />
																		</li></label></a>
																</s:iterator>
															</ul> <%-- <a href="#" onClick="geturl('<s:property value='al'/>')"> <s:property value='al'/></a> --%>
													</li>
													</a>
												</s:iterator>
											</ul></li>
			...
			<img alt="chgPass" src="icons/PasswordChange_new.png">
				...
				<button type="button" class="bicon" id="passchange"
					title="Change Password" onClick="funchgpass();">
					<img alt="passchange"
						src="<%=contextPath%>/icons/PasswordChange_new.png">
				</button>
			...
			<img alt="mail" src="icons/mail_new.png">
				...
				<button type="button" class="bicon" id="email" title="E-Mail"
					onclick="funSendEmail()">
			...
			<img alt="SMS" src="icons/SMS_new.png">
				...
				<button type="button" class="bicon" id="sms" title="SMS"
					onclick="funSendSms()">
					<img alt="SMS" src="<%=contextPath%>/icons/SMS_new.png">
				</button>

				<button id="iconym" class="animated wobble" hidden="true"
					title="Messenger" onclick="funMsngr();">
			...
			<img alt="Messenger" src="icons/support.png">
				...
				<button type="button" class="bicon" id="support"
					title="Need Help,Contact Support Center" onclick="funSupport()">
...
<button id="icon" name="clear"  title="Data Clear" onclick="funClear()">
			...
			<img alt="Approval" src="icons/app16.png">
				...
				<button type="button" class="bicon" id="approve"
					title="Executive Folio" onclick="funApprove()">
	    ...
	    <button id="icon" title="Move to Last Form">
		 ...
		 <button id="icon" title="Running Forms">
			<img alt="runningForms" src="icons/refresh_new.png">
				...
				<label class="space">&nbsp;</label> <label class="user">
					Welcome</label><label class="user1">&nbsp;&nbsp;<%=session.getAttribute("USERNAME")%></label>
				<label class="logTime"> <%=session.getAttribute("LOGGEDIN").toString()
					.substring(0, 19)%>
				</label> <label class="period">Period&nbsp;&nbsp;<input type="text"
					id="txtaccountperiodfrom" name="txtaccountperiodfrom" readonly
					style="height: 15px; border: none; text-align: center; background: #fff; width: 50%;"
					value='<%=session.getAttribute("STYEAR")%>'></label>&nbsp; <label
					class="period1">To&nbsp;&nbsp;<input type="text"
					id="txtaccountperiodto" name="txtaccountperiodto" readonly
					style="height: 15px; border: none; text-align: center; background: #fff; width: 50%;"
					value='<%=session.getAttribute("EDYEAR")%>'></label> <label
					class="licenceExpiry"><b><%=session.getAttribute("ERA")%></b></label>

				<label class="period1"> 
				<!-- <input type="text" id="txtSearchmenu"
					placeholder="Search Menu" id="txtSearchMenu" name="txtSearchMenu"
					style="height: 20px; text-align: center; background: #fff; width: 100%;"
					value="" /> -->
				</label>
	...
	<input type="hidden" id="backdateallowed" name="backdateallowed"
		value='<s:property value="backdateallowed"/>' />
	<input type="hidden" id="pdcascdcdateallowed"
		name="pdcascdcdateallowed"
		value='<s:property value="pdcascdcdateallowed"/>' />
	<input type="hidden" id="monthclosed" name="monthclosed"
		value='<s:property value="monthclosed"/>' />
	<input type="hidden" id="taxdateval" name="taxdateval"
		value='<s:property value="taxdateval"/>' />
	<input type="hidden" name="formcurrencytype" id="formcurrencytype"
		value='<s:property value="formcurrencytype"/>' />
	<input type="hidden" id="curdec" name="curdec"
		value='<s:property value="curdec"/>' />
	<input type="hidden" id="amtdec" name="amtdec"
		value='<s:property value="amtdec"/>' />
	<input type="hidden" id="chkexportdata" name="chkexportdata"
		value='<s:property value="chkexportdata"/>' />
	<div id="tt" class="easyui-tabs" style="width: 100%; height: 100%;">

		<div title="Home">
			<iframe scrolling="auto" frameborder="0"
				src="com/dashboard/dashBoard.jsp" style="width: 100%; height: 100%;"></iframe>
		</div>
	</div>
	<div id="windowcp">
		<div></div>

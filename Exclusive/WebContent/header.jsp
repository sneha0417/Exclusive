...
<link rel="stylesheet" type="text/css"
	href="<%=contextPath%>/css/loading.css">
<style>
.HeadIcons {
	font: 12px Tahoma;
	margin-top: 0px;
...
$(document).ready(function () {  
		...
		if($('#docno').val()!=''){
			var blur=$(this).attr('onblur');
			if(blur!=null && blur!="" && blur!="undefined" && typeof(blur)!="undefined"){
				if (blur.includes('funRoundAmt')) {
					$(this).val(parseFloat($(this).val()));
					if(blur.includes("getBaseAmountFrom")){
						getBaseAmountFrom();
					}
					else if(blur.includes("getBaseAmountTo")){
						getBaseAmountTo();
					}
				}		
			}
		...
		$('input[type="email"]').on('blur', function () {
			 ...
			 if(!emailReg.test(emailaddress))
				{ 
				 $('#valmsg').html('Enter Valid EmailID ! ');				 
   ...
   if(exefolio==1){
	...
	$("#txtaccperiodendyear").jqxDateTimeInput({ width: '125px', height: '15px', formatString:"dd.MM.yyyy"}); */
	  ...
	  $("#brchName").show();  $("#brchNames").hide(); $('#brchNames').attr('readonly', true ); $("#brchNames").val($("#brchName option:selected").text());$('#brchNames').removeClass();
	  $("#currency").show();  $("#currencys").hide(); $('#currencys').attr('readonly', true ); $("#currencys").val($("#currency option:selected").text());$('#currencys').removeClass();
...
<body onload="funChkButton();"
	onclick="getformbranch();getMessengerCount();">
	<div id="mainBG" class="homeContent" data-type="background">
		<div class="HeadIcons clearfix" id="full">
		 
			<font size=3px><label id="formdet" name="formdet"></label></font>

			<button type="button" class="icon" id="btnApproval"
				title="Document Status" onclick="funApproveBtn()" style="">
				<img alt="statusDocument"
					src="<%=contextPath%>/icons/approve_new.png">
			</button>
			<button type="button" class="icon" id="btnClose" title="Close Form"
				onclick="funCloseBtn()">
				<img alt="closeForm" src="<%=contextPath%>/icons/close_new.png">
			</button>
			<button type="button" class="icon" id="btnCreate"
				title="Create a new Document" onclick="funCreateBtn()">
				<img alt="newDocument" src="<%=contextPath%>/icons/add_new.png">
			</button>
			<button type="button" class="icon" id="btnEdit"
				title="Change current Document" onclick="funEditBtn()">
				<img alt="editDocument" src="<%=contextPath%>/icons/edit_new.png">
			</button>
			<button type="button" class="icon" id="btnPrint"
				title="Print current Document" onclick="funPrintBtn()">
				<img alt="printDocument" src="<%=contextPath%>/icons/print_new.png">
			</button>
			<button type="button" class="icon" id="btnExcel"
				title="Export current Document to Excel" onclick="funExcelBtn()">
				<img alt="excelDocument" src="<%=contextPath%>/icons/excel_new.png">
			</button>
			<button class="icon" id="btnDelete" title="Delete current Document">
				<img alt="deleteDocument"
					src="<%=contextPath%>/icons/delete_new.png">
			</button>
			...
			<button type="button" class="icon" id="btnCancel"
				title="Cancel Changes" onclick="funCancelBtn()" hidden="true">
				<img alt="cancelChanges" src="<%=contextPath%>/icons/cancel_new.png">
			</button>

			<button type="button" class="icon" id="btnSearch"
				title="Search a Document" onclick="funSearchBtn()">
				<img alt="searchDocument"
					src="<%=contextPath%>/icons/search_new.png">
			</button>

			<button type="button" class="icon" type="button" id="btnAttach"
				title="Attachment" onclick="funAttachBtn()">
				<img alt="Attachment"
					src="<%=contextPath%>/icons/attachment_new.png">
			</button>

			<button type="button" class="icon" type="button" id="btnCosting"
				title="Costing" onclick="funCostingBtn()">
				<img alt="Costing" src="<%=contextPath%>/icons/costtype.png">
			</button>

			<button type="button" class="icon" id="btnGuideLine"
				title="Guideline" onclick="funGuideLineBtn()">
				<img alt="Guideline" src="<%=contextPath%>/icons/guideline.png">
			</button>

			<button type="button" class="icon" id="btnSendmail"
				title="Send Document to Client" onclick="funSendMail()">
				<img alt="Sendmail" src="<%=contextPath%>/icons/mail_new.png">
			</button>

			<button type="button" class="icon" id="btnTerms"
				title="Terms and Conditions" onclick="funTermsCond()">
				<img alt="Terms" src="<%=contextPath%>/icons/tndc.png">
			</button>

			<label class="branch">Branch&nbsp;&nbsp; <select
				name="brchName" id="brchName" onChange="getCurr(this.value)"
				style="width: 150px;">
			</select>
			</label> <input type="text" name="brchNames" id="brchNames"
				readonly="readonly" value='<s:property value="brchNames"/>' /> <label
				class="currency">Currency&nbsp;&nbsp; <select
				name="currency" id="currency"
				onchange="getCurrencyType(this.value);" style="width: 50px;">
			</select></label> <input type="text" name="currencys" id="currencys"
				readonly="readonly" value='<s:property value="currencys"/>' />
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <label id="errormsg"
				name="errormsg"
				style="color: red; font-weight: bold; width: 100px; width: 250px;right: 75px;position: absolute;"><s:property
					value="errormsg" /></label> <label id="savemsg" name="savemsg"
				style="color: green; font-weight: bold;"></label> <input
				type="hidden" id="status" /> <input type="hidden" id="apprstatus" />
			<input type="hidden" id="isfirstappr" /> <input type="hidden"
				name="formdetail" id="formdetail"
				value='<s:property value="formdetail"/>' /> <input type="hidden"
				name="formdetailcode" id="formdetailcode"
				value='<s:property value="formdetailcode"/>' /> <input
				type="hidden" name="chkstatus" id="chkstatus"
				value='<s:property value="chkstatus"/>' /> <input type="hidden"
				id="termstatus" />
			<!-- 	If Add=1,Edit=2 -->
		...
		<!-- Modal alert for mobile validation -->
		<div class="modal fade" id="validation_modal" role="dialog">
		...
		<!-- Modal alert for number validation -->
</body>
</html>
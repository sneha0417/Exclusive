package com.dashboard.realestate.tenancycontractclose;
...
public class ClsTenancycontractcloseDAO {
	...
	public   JSONArray listsearch(String branch,String fromdate,String todate,String aa,String cldocno,String prdocno) throws SQLException {
...
*/
		...
		try {
				...
				String sql=" select m.voc_no,m.doc_no, m.date,m.ttype , m.cldocno,a.refname,pm.accname pname, "
						+ "  case when m.ttype=1 then 'Residence' when m.ttype=2 then 'Commercial'  when m.ttype=3 then 'Sub Lease' end   protype, if(m.Period=1,'Years','Months') Period, m.Period_no, m.Period_from, m.Period_to, m.not_Period  "
				+ " from rl_tncm  m  left join rl_propertymaster pm on pm.doc_no=m.prtype "
				+ " left join my_acbook a on a.acno=m.acno  where m.status=3 and m.clstatus=0 and m.Period_to<='"+sqltodate+"'   "+sqltest+" group by m.doc_no  ";

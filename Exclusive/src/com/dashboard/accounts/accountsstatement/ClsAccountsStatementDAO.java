...
public class ClsAccountsStatementDAO  { 
	...
	public JSONArray accountDetails(String type,String account,String partyname,String contact,String chk,String address) throws SQLException {
	    ...
	    try {
	            ...
	            if(!((address.equalsIgnoreCase("")) || (address.equalsIgnoreCase("0")))){
	                sql1=sql1+" and a.address like '%"+address+"%'";
	            }
				...
				if(type.equalsIgnoreCase("AR") || type.equalsIgnoreCase("AP")){
					sql = "select a.per_mob,a.mail1 email,t.doc_no,t.account,t.description,c.code curr,a.address,coalesce(pm.active,0) active from my_acbook a left join my_head t on a.acno=t.doc_no "
						+ ""+condition+" left join my_curr c on t.curid=c.doc_no left join rl_propertymaster pm on pm.acno=a.acno where t.atype='"+type+"' and a.status<>7 and t.m_s=0"+sql1 +" order by t.doc_no " ;
				}
	...
	public JSONArray accountsStatement(String branch,String fromdate,String todate,String accdocno,String check, String hidchckopnprint) throws SQLException {
		...
		try {
				...
				if(hidchckopnprint.equalsIgnoreCase("1")){
					opnchk=" union all select t.brhid,DATE_ADD('"+sqlFromDate+"',INTERVAL -1 DAY) trdate,"
							...
							+ "and t.acno= "+accdocno+" group by t.acno,t.curId ";
					}
				...
				sql = "select b.*,coalesce(round(@i:=@i+nettotal,2),0) balance from ( select a.trdate, a.brhid, a.transtype, a.description, a.ref_detail, a.tr_no, a.curId, a.currency, a.dramount, a.dr, a.cr, a.ldramount,"  
						...
						+ "'"+sqlFromDate+"' and  '"+sqlToDate+"' and trtype!=1 "+sql+" and t.acno= "+accdocno+" and t.yrid=0 "+
						opnchk+ ")t on h.doc_no=t.acno left join my_curr c on c.doc_no=t.curId order by acno,"
